...
async function newSeason(page) {
    ...
    $('#wrapper_bg > section > section.content_left > div > div.anime_name.new_series > div > div > ul > li').each((index, element) => {
        $elements = $(element)
...
async function search(query, page) {
    ...
    res = await axios.get(`https://gogoanime.vc//search.html?keyword=${query}&page=${page}`)
    const body = await res.data;
    const $ = cheerio.load(body)
...
async function tags(query, page) {
    ...
    $('#wrapper_bg > section > section.content_left > div > div.anime_name.anime_movies > div > div > ul > li').each((index, element) => {
        $elements = $(element)
        pageNo = $elements.find('a').html();
        pageNumbers.push(pageNo)
    })
